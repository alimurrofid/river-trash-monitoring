...
# Setup video path
video_path = "datasets/actioncam/testing_perhitungan_manual.mp4"
...
print(f"Effective frame size: {effective_width}x{effective_height}")
...
try:
    while cap.isOpened():
        ret, frame = cap.read()
        if not ret:
            print("📹 End of video reached! Restarting video...")
        ...
        fps = 1 / time_diff if prev_time > 0 and time_diff > 0 else 0
        prev_time = curr_time
        if fps > 0:
            fps_list.append(fps)
        ...
        # Create a clean frame for streaming (without overlays)
        stream_frame_clean = frame.copy()
        ...
        # Draw counting line on stream frame
        cv2.line(stream_frame_clean, (0, line_y), (effective_width, line_y), COLOR_LINE, 2)
        ...
        if results and len(results) > 0 and results[0].boxes is not None:
            for box in results[0].boxes:
                try:
                    ...
                    # Calculate size in cm - add safety checks
                    width_cm = w * cm_per_pixel if cm_per_pixel > 0 else 0
                    height_cm = h * cm_per_pixel if cm_per_pixel > 0 else 0
                    panjang_cm = max(width_cm, height_cm) if width_cm > 0 and height_cm > 0 else 0
                    ...
                    # Draw bounding boxes on both frames
                    cv2.rectangle(stream_frame_clean, (acc_x1, acc_y1), (acc_x2, acc_y2), color, 2)
                    cv2.rectangle(frame, (acc_x1, acc_y1), (acc_x2, acc_y2), color, 2)
                    
                    # Create simplified label for stream (class name and track ID only)
                    stream_label = f"{class_name}({track_id})"
                    cv2.putText(stream_frame_clean, stream_label, (acc_x1, acc_y1 - 10), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
                    
                    # Keep the full label for local viewing
                    full_label = f"{class_name}({track_id}) {confidence*100:.0f}% | {width_cm:.1f}x{height_cm:.1f}cm [{kategori}]"
                    cv2.putText(frame, full_label, (acc_x1, acc_y1 - 10), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
        ...
        # Add all the info overlays to the LOCAL display frame only
        cv2.line(frame, (0, line_y), (effective_width, line_y), COLOR_LINE, 2)
        
        # Display FPS and info on LOCAL frame only
        COLOR_FPS = (0, 255, 255)
        COLOR_TEXT = (255, 0, 0)
        COLOR_DISTANCE = (255, 255, 0)
        COLOR_CALIBRATION = (255, 0, 255)
        ...
        # Show counting results on LOCAL frame only
        y_offset = 180
        ...
        # Stream the CLEAN frame (only detection boxes and counting line) to RTMP
        if streaming_enabled:
            stream_frame(stream_frame_clean)
        ...
        if frame_count % 100 == 0:
            progress = (current_frame_idx / total_frames) * 100 if total_frames > 0 else 0
            print(f"Processing... Frame {current_frame_idx}/{total_frames} ({progress:.1f}%) - FPS: {fps:.1f}")
    ...
    for cls, counts in object_counter.items():
        ...
        print(f"  Rasio Makro:Meso = {counts['makro']}:{counts['meso']}" if counts['meso'] > 0 else f"  Rasio Makro:Meso = {counts['makro']}:0 (no meso detected)")
