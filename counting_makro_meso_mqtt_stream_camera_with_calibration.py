"""
Advanced real-time object detection system with network integration and streaming capabilities.
Combines YOLO detection, size measurement, camera calibration with MQTT publishing and RTMP streaming.
...
Features:
   ...
   - Automatic configuration via .env file
   - Real-time measurement categorization (meso/makro)

Network Integration:
   - MQTT: Publishes detection counts to configurable broker/topic
   - RTMP: Streams video feed to media server for remote monitoring
...
Output:
   - Real-time detection display with overlays
   - MQTT data publishing (plastic/nonplastic counts by size)
   - RTMP video streaming (clean feed without overlays)
