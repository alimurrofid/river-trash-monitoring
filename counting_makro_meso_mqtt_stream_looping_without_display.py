...
Features:
   ...
   - RTMP live streaming of processed video content
   - Automatic video looping for continuous processing
...
Input:
   - Video file: datasets/actioncam/testing_perhitungan_manual.mp4
   - Automatic looping when video ends

Network Integration:
   - MQTT: Publishes detection counts (plastic/nonplastic by size)
   - RTMP: Streams clean video feed without overlays
...
Output:
   - Processed video display with detection overlays
   - MQTT data publishing for remote monitoring
   - RTMP streaming of original video content
   - Final statistics with measurement logs and performance metrics
"""
import cv2
import numpy as np
import time
...
try:
    while cap.isOpened():
        ...
        # Keep original frame for streaming (plain video without overlays)
        ...
        # Stream the original frame (plain video without overlays) to RTMP
